...
scanning the full database. Sync still reads the complete selected spreadsheet
because the current `time_entries` tab is one canonical dataset; local indexes
cannot make that remote read smaller. An unchanged Drive modification time can
gate that full read out entirely.
...
peak memory, and the number of Sheets/Drive requests for each size:
...
If those measurements establish a need, retain `time_entries` as the active tab
and create append-only archive tabs named `time_entries_YYYY`. New and edited
entries would stay in the active tab until their calendar year is closed. A
verified migration would copy a year into its archive tab, retain its tombstones,
and write a versioned partition manifest in `config`.
...
interpret an incomplete active tab as the complete history.
