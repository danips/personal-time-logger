...
smaller. A provider change token can gate that full read out entirely. MySQL
and Cloudflare D1 use their API change sequence.
...
The cutoff must be based on Firefox profile storage and realistic API response
sizes.
...
baseline, not a live API payload or provider-latency benchmark.
...
each provider. MySQL and Cloudflare D1 would need an equivalent API/schema
partition contract. New and edited entries would stay in the active partition
until its calendar year is closed. A verified migration would copy a year into
the archive partition, retain its tombstones, and write a versioned partition
manifest in shared configuration.
