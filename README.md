...
10. Use **Send to Tempo** in the calendar to create the displayed week's completed worklogs directly in Tempo. Uncheck a day in the calendar header to leave it out of that send.
...
Each day header carries a checkbox that decides whether that day is part of the next send. All days start checked, so the default remains the whole displayed week, and the confirmation reports how many days are covered. Unchecking every day disables **Send to Tempo**. The selection is not stored: it resets to the full week whenever you change weeks, which keeps a resend after a partial send an explicit choice. Sending a day that was already sent still creates duplicates, so use this to send the remaining days rather than to correct an earlier send.
